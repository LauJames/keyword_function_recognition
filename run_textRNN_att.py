...
from model.textRNN_att import TextRNNAtt
...
def evaluate(x_dev, x_mask_dev, y_dev, sess, model, merged_summary=None, dev_writer=None, total_batch=None):
# def evaluate(x_dev, y_dev, sess, model):
    ...
    # for x_batch_eval, y_batch_eval in batch_eval:
        ...
        # summary, loss, accuracy = sess.run([merged_summary, model.loss, model.accuracy], feed_dict)
        loss, accuracy = sess.run([model.loss, model.accuracy], feed_dict)
        total_loss += loss * batch_len
        total_acc += accuracy * batch_len
        # dev_writer.add_summary(summary, total_batch)
...
def train():
    ...
    writer = tf.summary.FileWriter(tensorboard_dir + '/train')
    dev_writer = tf.summary.FileWriter(tensorboard_dir + '/dev')
    ...
    for epoch in range(args.epochs):
        ...
        for x_batch, x_mask_batch, y_batch in batch_train:
            ...
            if total_batch % args.evaluate_every == 0:
                ...
                # loss_dev, acc_dev = evaluate(x_dev, x_mask_dev, y_dev, session, merged_summary, dev_writer, total_batch, model=model)
